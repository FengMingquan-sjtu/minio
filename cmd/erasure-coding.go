...
import (
	...
	"github.com/FengMingquan-sjtu/xrs"
	"github.com/minio/minio/internal/logger"
)

// Erasure - erasure encoding details.
type Erasure struct {
	encoder                  func() xrs.Encoder
...
func NewErasure(ctx context.Context, dataBlocks, parityBlocks int, blockSize int64) (e Erasure, err error) {
	...
	if dataBlocks <= 0 || parityBlocks < 0 {
		return e, xrs.ErrInvShardNum
	}

	if dataBlocks+parityBlocks > 256 {
		return e, xrs.ErrMaxShardNum
	}
	...
	var enc xrs.Encoder
	var once sync.Once
	e.encoder = func() xrs.Encoder {
		once.Do(func() {
			e, err := xrs.New(dataBlocks, parityBlocks)
			if err != nil {
	...
	for algo := invalidErasureAlgo + 1; algo < lastErasureAlgo; algo++ {
		for _, conf := range testConfigs {
			...
			for i, data := range encoded {
				...
				//fmt.Fprintf(os.Stderr, "[d:%d,p:%d]: want %#v, got %#v\n", conf[0], conf[1], data, testData[i])
				failOnErr(err)
			...
			if a, b := want[conf], got[conf]; !reflect.DeepEqual(a, b) {
				//continue
				fmt.Fprintf(os.Stderr, "%v: error on self-test phase1 [d:%d,p:%d]: want %#v, got %#v\n", algo, conf[0], conf[1], a, b)
				ok = false
			...
			if a, b := first, encoded[0]; !bytes.Equal(a, b) {
				fmt.Fprintf(os.Stderr, "%v: error on self-test phase2 [d:%d,p:%d]: want %#v, got %#v\n", algo, conf[0], conf[1], hex.EncodeToString(a), hex.EncodeToString(b))
				//continue
				ok = false
