...
func NewErasure(ctx context.Context, dataBlocks, parityBlocks int, blockSize int64) (e Erasure, err error) {
	...
	if dataBlocks <= 0 || parityBlocks < 0 {
		return e, sxor.ErrInvShardNum
	}

	if dataBlocks+parityBlocks > 256 {
		return e, sxor.ErrMaxShardNum
	}
	...
	// Encoder when needed.
	var enc sxor.Encoder
	var once sync.Once
	e.encoder = func() sxor.Encoder {
		once.Do(func() {
			e, err := sxor.New(dataBlocks, parityBlocks)
			if err != nil {
